  ...
  const pokemonList = await Promise.all(
    favPokemon.map(async (fav) => {
      ...
      if (pokemonData.count > 1) {
        const exactPokemon = pokemonData.results.find((result) => result.pokemon === fav)
...
export const fetchSearchPokemon = (pokemonName) => {
  ...
  return fetch(`https://pokedex-alchemy.herokuapp.com/api/pokedex?pokemon=${lowerCaseName}`)
    .then((data) => data.json())
    .then((pokemonData) => {
...
export const fetchFilteredPokemon = async (type) => {
  ...
  const filteredPokemon = pokemonData.results.map((pokemon) => pokeMunger(pokemon))
...
export const fetchSortedPokemon = async ({ sort, type }) => {
  ...
  const pokemonData = await res.json()
  const sortedPokemon = pokemonData.results.map((pokemon) => pokeMunger(pokemon))
